...
class Cube:
    def __init__(self, parent, x, y, z, w, active):
        self.parent = parent
    ...
    def rich_repr(self):
        return f"<Cube {repr(self)} @ {(self.x, self.y, self.z, self.w)}>"
        ...
        for z_idx, z in enumerate(matrix_3d):
            for y_idx, y in enumerate(z):
                for x_idx, x in enumerate(y):
                    cube = Cube(self, x_idx, y_idx, z_idx, 0, True if x == "#" else False)
                    self.set_grid(x_idx, y_idx, z_idx, 0, cube)

        self.loops = loops

    def set_grid(self, x, y, z, w, value):
        self.grid[f"{z}:{y}:{x}:{w}"] = value

    def get_grid(self, x, y, z, w, make_neighbours=False):
        idx = f"{z}:{y}:{x}:{w}"
        if idx not in self.grid:
            if make_neighbours:
                self.grid[idx] = Cube(self, x, y, z, w, False)
            else:
                return None
